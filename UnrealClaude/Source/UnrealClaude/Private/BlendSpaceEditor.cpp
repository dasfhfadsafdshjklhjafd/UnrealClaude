...
TSharedPtr<FJsonObject> FBlendSpaceEditor::SetAxisInternal(UBlendSpace* BS, int32 AxisIndex, const TSharedPtr<FJsonObject>& AxisParams)
{
	...
	if (AxisParams->TryGetStringField(TEXT("axis_name"), StrVal)) { Param.DisplayName = StrVal; }
	if (AxisParams->TryGetStringField(TEXT("name"), StrVal))      { Param.DisplayName = StrVal; }
	if (AxisParams->TryGetNumberField(TEXT("min"), NumVal))       { Param.Min = static_cast<float>(NumVal); }
	if (AxisParams->TryGetNumberField(TEXT("max"), NumVal))       { Param.Max = static_cast<float>(NumVal); }
	if (AxisParams->TryGetNumberField(TEXT("grid_divisions"), NumVal)) { Param.GridNum = static_cast<int32>(NumVal); }
	if (AxisParams->TryGetBoolField(TEXT("snap_to_grid"), BoolVal))   { Param.bSnapToGrid = BoolVal; }
	if (AxisParams->TryGetBoolField(TEXT("wrap_input"), BoolVal))     { Param.bWrapInput = BoolVal; }

	if (AxisParams->TryGetNumberField(TEXT("interp_time"), NumVal))   { Interp.InterpolationTime = static_cast<float>(NumVal); }
	...
	if (AxisParams->TryGetNumberField(TEXT("damping_ratio"), NumVal)) { Interp.DampingRatio = static_cast<float>(NumVal); }
	if (AxisParams->TryGetNumberField(TEXT("max_speed"), NumVal))     { Interp.MaxSpeed = static_cast<float>(NumVal); }
...
TSharedPtr<FJsonObject> FBlendSpaceEditor::ExecuteBatch(UBlendSpace* BS, const TArray<TSharedPtr<FJsonValue>>& Operations)
{
	...
	for (int32 i = 0; i < Operations.Num(); ++i)
	{
		...
		if (bSuccess) OKCount++; else ErrCount++;
	}
