...
{
	// sync_mesh_bones requires the SkeletonModifier plugin (USkeletonModifier).
	// That plugin is not included in the base engine and must be installed separately.
	// To enable this operation, add "SkeletonModifier" to Build.cs PrivateDependencyModuleNames
	// and uncomment the full implementation below.
	return FMCPToolResult::Error(TEXT("sync_mesh_bones requires the SkeletonModifier plugin, which is not available in this build."));
}
