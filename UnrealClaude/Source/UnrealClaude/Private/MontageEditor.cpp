...
{
	if (!Montage) { OutError = TEXT("Montage is null"); return false; }
...
bool FMontageEditor::AddSegment(UAnimMontage* Montage, int32 SlotIndex, const FString& AnimSequencePath, float StartPos, FString& OutError)
{
	if (!Montage) { OutError = TEXT("Montage is null"); return false; }
	...
	if (!AnimSeq) { OutError = LoadError; return false; }
...
bool FMontageEditor::SetSegmentProperties(UAnimMontage* Montage, int32 SlotIndex, int32 SegmentIndex,
	TOptional<float> PlayRate, TOptional<float> AnimStartTime, TOptional<float> AnimEndTime,
	TOptional<float> StartPos, const FString& AnimSequencePath, FString& OutError)
{
	if (!Montage) { OutError = TEXT("Montage is null"); return false; }
	...
	if (PlayRate.IsSet())     Seg.AnimPlayRate  = PlayRate.GetValue();
	if (AnimStartTime.IsSet()) Seg.AnimStartTime = AnimStartTime.GetValue();
	if (AnimEndTime.IsSet())   Seg.AnimEndTime   = AnimEndTime.GetValue();
	if (StartPos.IsSet())      Seg.StartPos       = StartPos.GetValue();
...
bool FMontageEditor::AddNotify(UAnimMontage* Montage, const FString& NotifyName, float TriggerTime,
	int32 TrackIndex, const FString& NotifyClassPath, FString& OutError)
{
	if (!Montage) { OutError = TEXT("Montage is null"); return false; }
	...
	NewNotify.TriggerTimeOffset = GetTriggerTimeOffsetForType(Montage->CalculateOffsetForNotify(TriggerTime));
...
bool FMontageEditor::AddNotifyState(UAnimMontage* Montage, const FString& NotifyStateName,
	float StartTime, float Duration, int32 TrackIndex, const FString& NotifyStateClassPath, FString& OutError)
{
	if (!Montage) { OutError = TEXT("Montage is null"); return false; }
	...
	NewNotify.TriggerTimeOffset = GetTriggerTimeOffsetForType(Montage->CalculateOffsetForNotify(StartTime));
	NewNotify.TrackIndex = TrackIndex;
	...
	if (StateInstance)
	{
		...
		NewNotify.RefreshEndTriggerOffset(Montage->CalculateOffsetForNotify(NewNotify.EndLink.GetTime()));
...
bool FMontageEditor::RemoveNotify(UAnimMontage* Montage, const FString& NotifyName, FString& OutError)
{
	...
	{
		if (Montage->Notifies[i].NotifyName == SearchName) { FoundIdx = i; break; }
	}
	...
	const TSharedPtr<FJsonObject>& Properties, FString& OutError)
{
	...
	if (!SetPropertiesOnObject(Target, Properties, OutError)) return false;
...
bool FMontageEditor::SetPropertiesOnObject(UObject* Object, const TSharedPtr<FJsonObject>& Properties, FString& OutError)
{
	if (!Object || !Properties.IsValid()) { OutError = TEXT("Null object or properties"); return false; }
	...
	for (const auto& Pair : Properties->Values)
	{
		...
		{
			bool bVal = false;
			if (!Value->TryGetBool(bVal)) { OutError = FString::Printf(TEXT("Property '%s' expects bool"), *Key); return false; }
			BoolProp->SetPropertyValue(PropAddr, bVal);
		}
		else if (FNumericProperty* NumProp = CastField<FNumericProperty>(Prop))
		{
			double NumVal = 0.0;
			if (!Value->TryGetNumber(NumVal)) { OutError = FString::Printf(TEXT("Property '%s' expects number"), *Key); return false; }
			if (NumProp->IsFloatingPoint()) NumProp->SetFloatingPointPropertyValue(PropAddr, NumVal);
			else NumProp->SetIntPropertyValue(PropAddr, static_cast<int64>(NumVal));
		}
		else if (FNameProperty* NameProp = CastField<FNameProperty>(Prop))
		{
			FString StrVal;
			if (!Value->TryGetString(StrVal)) { OutError = FString::Printf(TEXT("Property '%s' expects string"), *Key); return false; }
		...
		{
			FString StrVal;
			if (!Value->TryGetString(StrVal)) { OutError = FString::Printf(TEXT("Property '%s' expects string (enum name)"), *Key); return false; }
			UEnum* Enum = EnumProp->GetEnum();
			int64 EnumVal = Enum->GetValueByNameString(StrVal);
			if (EnumVal == INDEX_NONE) { OutError = FString::Printf(TEXT("Enum value '%s' not found in %s"), *StrVal, *Enum->GetName()); return false; }
		...
		else if (FByteProperty* ByteProp = CastField<FByteProperty>(Prop))
		{
			if (ByteProp->Enum)
			{
				...
				if (EnumVal == INDEX_NONE) { OutError = FString::Printf(TEXT("Enum value '%s' not found"), *StrVal); return false; }
		...
		{
			OutError = FString::Printf(TEXT("Unsupported property type '%s' for '%s'"), *Prop->GetCPPType(), *Key);
			return false;
		}
	}
...
int32 FMontageEditor::CleanupNotifyTracks(UAnimMontage* Montage)
{
	...
	{
		if (Notify.TrackIndex > MaxUsedTrack) MaxUsedTrack = Notify.TrackIndex;
	}

	int32 DesiredCount = FMath::Max(1, MaxUsedTrack + 1);
